...
public class GcmIntentService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        Log.v("GCMIntent","onHandleIntent Key "+key);
        switch (key) {
            case SUBSCRIBE:
    ...
    private void registerGCM() {
        ...
        try {
            ...
            StatiConstants.Gcm = token;


            // sending the registration id to our server
            sendRegistrationToServer(token);

            sharedPreferences.edit().putBoolean(NotifyConfig.SENT_TOKEN_TO_SERVER, true).apply();
        } catch (Exception e) {
            Log.e(TAG, "Failed to complete token refresh", e);

            sharedPreferences.edit().putBoolean(NotifyConfig.SENT_TOKEN_TO_SERVER, false).apply();
        }
