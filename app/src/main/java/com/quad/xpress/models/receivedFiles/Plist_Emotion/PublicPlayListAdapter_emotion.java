...
public class PublicPlayListAdapter_emotion extends RecyclerView.Adapter<PublicPlayListAdapter_emotion.MyViewHolder> {
    ...
    public PublicPlayListAdapter_emotion(List<PlayListitems_emotion> rvListitems, Context _context, OnRecyclerListener recyclerListener) {
       ...
       // Toast.makeText(_context,"tag_store"+tag_store[1],Toast.LENGTH_LONG).show();
        ...
        try {
            if (list.getFileMimeType().equals("audio/mp3")) {
               ...
               // holder.Play_icon.getRootView().setVisibility(View.INVISIBLE);
                Glide.with(_context).load(R.drawable.ic_mic_placeholder).bitmapTransform(new RoundedCornersGlideImage(this._context,15,2))
                ...
                Log.v("", "TBPath " + TBPath);
                Glide.with(_context).load(TBPath).bitmapTransform(new RoundedCornersGlideImage(this._context,15,2)).centerCrop().into(holder.RvImage);
